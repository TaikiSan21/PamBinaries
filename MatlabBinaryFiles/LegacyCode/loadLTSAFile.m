function [ltsas fileHeader fileFooter moduleheader modulefooter] = loadLTSAFile(fileName, arg2, arg3, arg4)
ltsas = [];
fileHeader = [];
fileFooter = [];
moduleheader = [];
modulefooter = [];
if nargin == 0
    error('A file name is required by loadLTSAfile')
end
if nargin < 2
    preallocate = 0;
    startDate = 0;
...
try
    ...
    while (1)
        ...
        switch nextType
            ...
            case -3
                moduleheader = readLTSAModuleHeader(f);
...
function header = readLTSAModuleHeader(file);
